...
class ReportController extends Controller
{
    ...
    public function index(ReportDataTable $dataTable, Request $request, Report $report)
    {
        ...
        $data['request'] = $request;
        return $dataTable->addScope(new \App\DataTables\Scopes\BaseDataTableScope)->render('employee.reports.index', $data);
    }
    ...
    public function create(Report $Report,Request $request )
    {
        ...
        return view('employee.reports.create', $data);
    }
    ...
    public function getReportitems(Request $request)
    {
        ...
        return view("employee.reports.partials.reportitems", $data);
    }
...
//        }
    ...
    public function getReportitemForm(Request $request)
    {
        ...
        $this->_append_form_variables($data);
        $data['route'] = $Reportitem->id ? 'employee.report.updateReportitem' : 'employee.report.storeReportitem';
        ...
        $data['latestProjects'] = DB::select(DB::raw("SELECT project_name, project_id, created_at FROM
            (SELECT c.name AS project_name, c.id AS project_id, a.created_at as created_at
            FROM reportitems a
            JOIN reports b
            ON b.id = a.report_id
            JOIN projects c
        ...
        return view('employee.reports.partials.reportform', $data);
    }
    ...
    public function releaselockbreak(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            return view('employee.reports.partials.releaselock_break', $data);
        }
    }
    ...
    public function getgithubCommits(Request $request)
    {
        ...
        return view('employee.reports.partials.github_latest_commits', $data);
    }
    ...
    protected function checkReportForCompensation($Report)
    {
        ...
        if (in_array($date, $holidays) || Carbon::parse($date)->dayOfWeek == 0 || Setting::isOfficialLeaveToday($date)) {
            ...
            if(strtotime($lastreport->end) < strtotime($actualEndtime)){
                if ((strtotime($worked_hours) < strtotime($official_worked_hours))) {
                    ...
                    if ($worked_hours >= $without_permission_hours) {
                        // 7:00 >= (8:10 - 2:00 == 6:10)
                        $msg_text = "Worked less than  $official_worked_hours";
                        $entryHelper->addPermission($lastreport->end, $actualEndtime, $msg_text);
                    } else {
    ...
    public function show(Report $Report)
    {
        ...
        return view('employee.reports.view', $data);
    }
    ...
    public function edit($id)
    {
        ...
        $data['formUrl'] = route('employee.report.update', $data['Report']->id);
        $data['formMethod'] = 'PUT';

        return view('employee.reports.edit', $data);
    }
    ...
    public function extendhours(Request $request){
        ...
        $data['status'] = ReportItem::$status;
        return view('employee.reports.partials.extend',$data);
    }
}

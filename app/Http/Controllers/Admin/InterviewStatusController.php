<?php

namespace App\Http\Controllers\Admin;

use App\DataTables\InterviewStatusDataTable;
use App\Http\Controllers\Controller;
use App\Models\Interviewstatus;
use Illuminate\Http\Request;
use Illuminate\Http\Response;
use Illuminate\Support\Facades\Validator;
use function flash;
use function redirect;
use function view;

class InterviewStatusController extends Controller {

    /**
     * Display a listing of the resource.
     *
     * @return Response
     */
    public function index(InterviewStatusDataTable $dataTable) {
        
        return $dataTable->addScope(new \App\DataTables\Scopes\BaseDataTableScope)->render('admin.interviewstatus.index');
    }

    /**
     * Show the form for creating a new resource.
     *
     * @return Response
     */
    public function create() {
        return view('admin.interviewstatus.create');
    }

    /**
     * Store a newly created resource in storage.
     *
     * @param  Request  $request
     * @return Response
     */
    public function store(Request $request,Interviewstatus $Interviewstatus) {
        $this->_validate($request);
        
        $this->_save($request, $Interviewstatus);
        if ($request->ajax()) {
            return response()->json(['success' => 'Success!'], 200);
        } else {
            flash('Status created successfully')->success();
            return redirect()->route('interviewstatus.index');
        }
    }

    private function _save($request, $Interviewstatus) {
        $data = $request->except(['_token']);
        $Interviewstatus->fill($data);
        $Interviewstatus->save();
    }

    /**
     * Display the specified resource.
     *
     * @param  int  $id
     * @return Response
     */
    public function show(Interviewstatus $Interviewstatus) {
        $Model = $Interviewstatus;
        return view('admin.interviewstatus.view', compact('Model'));
    }

    /**
     * Show the form for editing the specified resource.
     *
     * @param  int  $id
     * @return Response
     */
    public function edit(Interviewstatus $Interviewstatus) {
        $Model = $Interviewstatus;
        return view('admin.interviewstatus.edit', compact('Model'));
    }

    /**
     * Update the specified resource in storage.
     *
     * @param  Request  $request
     * @param  int  $id
     * @return Response
     */
    public function update(Request $request, Interviewstatus $Interviewstatus) {
        $this->_validate($request, $Interviewstatus->id);

        $this->_save($request, $Interviewstatus);

        flash('Status Updated successfully')->success();
        return redirect()->route('interviewstatus.index');
    }

    /**
     * Remove the specified resource from storage.
     *
     * @param  int  $id
     * @return Response
     */
    public function destroy(Interviewstatus $Interviewstatus) {
        $Interviewstatus->delete();
    }

    private function _validate($request, $id = null) {
        $rules = [
            'name' => "required|unique:interview_status,name,$id,id,deleted_at,NULL",
        ];
        $this->validate($request, $rules);
    }
    
    public function active(Request $request, $id) {
        $model = Interviewstatus::find($id);
        $model->active = $request->active;
        $model->save();
        return response()->json(['success' => 'Success!', 'active' => $model->active], 200);
    }

}

<?php

namespace App\Http\Controllers\Admin;

use App\DataTables\TechnologyDataTable;
use App\Http\Controllers\Controller;
use App\Models\Technology;
use Illuminate\Http\Request;
use Illuminate\Http\Response;
use function flash;
use function redirect;
use function view;

class TechnologyController extends Controller {

    /**
     * Display a listing of the resource.
     *
     * @return Response
     */
    public function index(TechnologyDataTable $dataTable) {
        return $dataTable->addScope(new \App\DataTables\Scopes\BaseDataTableScope)->render('admin.technologies.index');
    }

    /**
     * Show the form for creating a new resource.
     *
     * @return Response
     */
    public function create() {
        return view('admin.technologies.create');
    }

    /**
     * Store a newly created resource in storage.
     *
     * @param  Request  $request
     * @return Response
     */
    public function store(Request $request, Technology $Technology) {
        $this->_validate($request);

        $this->_save($request, $Technology);

        flash('Technology created successfully')->success();
        return redirect('admin/technology');
    }

    private function _save($request, $Technology) {
        $data = $request->except(['_token']);
        $Technology->fill($data);
        $Technology->save();
    }

    /**
     * Display the specified resource.
     *
     * @param  int  $id
     * @return Response
     */
    public function show(Technology $Technology) {

        return view('admin.technologies.view', compact('Technology'));
    }

    /**
     * Show the form for editing the specified resource.
     *
     * @param  int  $id
     * @return Response
     */
    public function edit(Technology $Technology) {
        
        return view('admin.technologies.edit', compact('Technology'));
    }

    /**
     * Update the specified resource in storage.
     *
     * @param  Request  $request
     * @param  int  $id
     * @return Response
     */
    public function update(Request $request, Technology $Technology) {
        $this->_validate($request);

        $this->_save($request, $Technology);

        flash('Technology Updated successfully')->success();
        return redirect('admin/technology');
    }

    /**
     * Remove the specified resource from storage.
     *
     * @param  int  $id
     * @return Response
     */
    public function destroy(Technology $Technology) {
        $Technology->delete();
    }

    private function _validate($request, $id = null) {
        $rules = [
            'name' => 'required',
        ];
        $this->validate($request, $rules);
    }
    
    public function active(Request $request, $id) {
        $model = Technology::find($id);
        $model->active = $request->active;
        $model->save();
        return response()->json(['success' => 'Success!', 'active' => $model->active], 200);
    }

}

...
<div class="form-body">
        ...
        <div class="col-md-12">
            <div class="form-group">
                <h5 class="box-title">Select Employees</h5>
                 {{ Form::select('value[employee_id][]', $Employees, @$values['employee_id'] ? @$values['employee_id'] : @$Employee, ['class' => 'form-control searchablemultiselect', 'multiple' => true]) }}
            </div>
        </div>
    </div>
    <hr>
    ...
    <div id="default-slots" class="slots-div {{ @$Model->isAllday ? '' : 'hide' }}">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                {{ Form::select("value[default_slot]", $slots, @$values['slots'][1], ['class' => 'form-control', 'placeholder' => 'Select']) }}
                @foreach(range(1, 31) as $day)
                    {{ Form::hidden("value[slots][$day]", @$values['slots'][$day] , ['class' => 'custom-slots', 'id' => 'day-'.$day]) }}
                @endforeach
            </div>
        </div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('select[name="value[default_slot]"]').on('change', function () {
            ...
            CalendarApp.prototype.init = function () {
                ...
                $this.$calendarObj = $this.$calendar.fullCalendar({
                    ...
                    events: function (start, end, timezone, callback) {
                        $.ajax({
                            url: '{{ route('schedule.slot_events') }}',
                            method: 'POST',
                            // dataType: 'xml',
                            data: {

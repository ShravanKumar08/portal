    ...
    @if(@$Report)
        <script type="text/javascript">
            $(document).ready(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data['success'] == 'Success') {
                                ...
                                $('.changeTimeFormat').trigger('change');
                ...
                $('body').on('click', '#items tbody tr .btn-delete', function (e) {
                    ...
                    swal({
                        ...
                        $.ajax({
                            method: "DELETE",
                            url: "{{ route('employee.report.deleteReportitems') }}",
                            data: {id: id},
                            dataType: 'json',
                            success: function (data) {
            ...
            function initSortable()
            {
                ...
                $( "#items tbody" ).sortable({
                    update:function(event,ui){
                        ...
                        $.ajax({
                            url:"{{ route('employee.report.order.edit') }}",
                            method:'post',
                            dataType:"json",
                            data:{'id':positions},
            ...
            $('body').on('shown.bs.modal', '#extendModal', function (e) {
                ...
                $.ajax({
                    method: 'GET',
                    url: "{{ route('employee.report.extendhours') }}",
            ...
            // When we click the latest project label
            $('body').on('click', '.project-label', function (e) {
                ...
                let $projectElem = $('#project-remote-select2');

                if ($projectElem.find("option[value='" + data.id + "']").length == 0) {
                    // Create a DOM Option and pre-select by default
                    var newOption = new Option(data.text, data.id, true, true);
                    // Append it to the select
                    $projectElem.append(newOption).trigger('change').val(data.id);
                }

                $projectElem.val(data.id).trigger('change');
            ...
            function getReportitems() {
                ...
                $.ajax({
                    method: 'get',
                    url: "{{ route('employee.report.getReportitems') }}",
                    dataType: 'html',
                    data: {'id': "{{ @$Report->id }}"},
                    success: function (data) {
            ...
            function initelement() {
                ...
                $("#project-remote-select2").select2({
                    ...
                    ajax: {
                        url: '{{ route('employee.report.searchproject') }}',
                        dataType: 'json',
                        type: "GET",
                        quietMillis: 50,
                        data: function (term) {
                ...
                $('#project-remote-select2').on('select2:select', function (e) {
                    ...
                    $.ajax({
                        url: '{{ route('employee.report.searchtechnolgy') }}',
                        type: 'POST',
                        data: {
                            'value': value
                        },

                        ...
                        <table class="display nowrap table table-hover table-striped table-bordered dataTable no-footer ">
                            <thead >
                                <tr>
                                    ...
                                    <th>Status</th>
                                    <th>{{$lecturer_id->employee_id == \Auth::user()->employee->id ? 'Mark Attendance' : 'Attendance'}}</th>
                                </tr>
                            </thead>
                            ...
                            <tr>
                                ...
                                @endif
                                @if($lecturer_id->employee_id == \Auth::user()->employee->id)
                                <td>
                                    <div class="switch">
                                        <label>
                                ...
                                @else
                                <td>
                                    @if($lecture->pivot->mark_attendance == 1)
...
<script>
    ...
    $(document).ready(function(){
        $('input[type="checkbox"]').change(function(){
            ...
            $.ajax({
                method: 'POST',
                url: "{{ route('employee.lectures.markattendance') }}",
                data: {
                    'attendance': present,
                    'joiner_id':joiner, 
                    'lecture_id':lecturer,
